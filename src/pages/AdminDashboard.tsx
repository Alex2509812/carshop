...
import {
  ...
  ShoppingBag, DollarSign, ImageIcon, Pencil
} from 'lucide-react';
...
const AdminDashboard = () => {
  ...
  const abrirEdicion = (prod: Producto) => {
    ...
    setNuevoProd({ nombre: prod.nombre, precio: prod.precio, categoria: prod.categoria, imagen: prod.imagen, detalles: prod.detalles, stock: prod.stock });
    setError(null);
    setShowModal(true);
  };
  ...
  const guardarProducto = async (e: React.FormEvent) => {
    ...
    if (!nuevoProd.nombre.trim()) { setError('El nombre del producto es obligatorio.'); setIsSaving(false); return; }
    if (nuevoProd.precio <= 0) { setError('El precio debe ser mayor a $0.'); setIsSaving(false); return; }

    try {
      if (editandoId !== null) {
        const { error } = await supabase.from('productos').update({
          nombre: nuevoProd.nombre.trim(), precio: nuevoProd.precio, categoria: nuevoProd.categoria,
          imagen: nuevoProd.imagen.trim(), detalles: nuevoProd.detalles.trim(), stock: nuevoProd.stock,
        }).eq('id', editandoId);
    ...
    <div style={{ minHeight: '100vh', background: '#0f1117', color: '#e8e8e8', fontFamily: "'DM Sans', sans-serif", padding: '2rem 1.5rem' }}>
      <div style={{ maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <button onClick={abrirNuevo} style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#4a6cf7', color: '#fff', border: 'none', borderRadius: 10, padding: '10px 20px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
        ...
        <div style={{ background: '#1a1d27', borderRadius: 16, border: '1px solid rgba(255,255,255,0.06)', overflow: 'hidden' }}>
          ...
          {loading ? (
            ...
            <div style={{ padding: '3rem', textAlign: 'center', color: '#555', fontSize: 14 }}>No hay productos aún.</div>
          ) : (
                ...
                <tbody>
                  {productos.map((prod, i) => (
                    <tr key={prod.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.01)' }}>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <span style={{ color: prod.stock === 0 ? '#ef4444' : prod.stock < 5 ? '#f59e0b' : '#22c55e', fontWeight: 700 }}>{prod.stock}</span>
                      </td>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <div style={{ display: 'flex', gap: 8 }}>
                          <button onClick={() => abrirEdicion(prod)} style={{ background: 'rgba(74,108,247,0.1)', color: '#4a6cf7', border: '1px solid rgba(74,108,247,0.2)', borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
                            <Pencil size={13} /> Editar
                          </button>
                          <button onClick={() => eliminarProducto(prod.id)} style={{ background: 'rgba(239,68,68,0.1)', color: '#ef4444', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5 }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
          <div style={{ background: '#1a1d27', borderRadius: 20, border: '1px solid rgba(255,255,255,0.08)', width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto', padding: '2rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
              <div>
                <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: '#fff' }}>{editandoId !== null ? 'Editar Producto' : 'Nuevo Producto'}</h2>
                <p style={{ margin: 0, fontSize: 12, color: '#555', marginTop: 2 }}>{editandoId !== null ? 'Modifica los campos necesarios' : 'Completa todos los campos'}</p>
              </div>
            ...
            <form onSubmit={guardarProducto} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                ...
                <input style={inputStyle} type="text" placeholder="Ej: Shampoo pH Neutro 1L" value={nuevoProd.nombre} onChange={e => setNuevoProd({ ...nuevoProd, nombre: e.target.value })} required />
              </div>
              <div>
                <label style={labelStyle}>Descripción / Detalles</label>
                <textarea style={{ ...inputStyle, minHeight: 80, resize: 'vertical' }} placeholder="Ej: Fórmula concentrada..." value={nuevoProd.detalles} onChange={e => setNuevoProd({ ...nuevoProd, detalles: e.target.value })} />
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div>
                  <label style={labelStyle}>Categoría *</label>
                  <select style={inputStyle} value={nuevoProd.categoria} onChange={e => setNuevoProd({ ...nuevoProd, categoria: e.target.value })}>
                ...
                <div>
                  ...
                  <input style={inputStyle} type="number" min={0} placeholder="0" value={nuevoProd.stock} onChange={e => setNuevoProd({ ...nuevoProd, stock: Number(e.target.value) })} required />
                </div>
              </div>
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  ...
                  <input style={{ ...inputStyle, paddingLeft: 28 }} type="number" min={0} placeholder="0" value={nuevoProd.precio || ''} onChange={e => setNuevoProd({ ...nuevoProd, precio: Number(e.target.value) })} required />
                </div>
              </div>
              <div>
                <label style={labelStyle}>URL de la imagen</label>
                <input style={inputStyle} type="url" placeholder="https://images.unsplash.com/..." value={nuevoProd.imagen} onChange={e => setNuevoProd({ ...nuevoProd, imagen: e.target.value })} />
                {nuevoProd.imagen && (
                  <img src={nuevoProd.imagen} alt="preview" style={{ marginTop: 8, width: '100%', height: 120, objectFit: 'cover', borderRadius: 10, border: '1px solid rgba(255,255,255,0.08)' }} onError={e => (e.currentTarget.style.display = 'none')} />
                )}
              </div>
              <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
                <button type="submit" disabled={isSaving} style={{ flex: 1, background: isSaving ? '#333' : editandoId !== null ? '#22c55e' : '#4a6cf7', color: '#fff', border: 'none', borderRadius: 10, padding: '12px', fontSize: 14, fontWeight: 700, cursor: isSaving ? 'not-allowed' : 'pointer' }}>
                  {isSaving ? 'Guardando...' : editandoId !== null ? 'Guardar Cambios' : 'Guardar Producto'}
                </button>
                <button type="button" onClick={() => setShowModal(false)} style={{ background: 'rgba(255,255,255,0.06)', color: '#888', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: '12px 20px', fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
                  Cancelar
                </button>
              </div>
            </form>
